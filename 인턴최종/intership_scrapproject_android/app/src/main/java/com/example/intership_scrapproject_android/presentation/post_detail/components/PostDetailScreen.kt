            ...
            PostDetailAppBar(
                ...
                onLinkButtonPress = {
                    if (post == null || post.link.isEmpty() ){
                        viewModel.onEvent(PostDetailEvent.EmptyLinkDetect)
                    }
                    else {
                        navController.currentBackStackEntry?.savedStateHandle?.set("link", post.link)
                        navController.navigate(NavHostRoute.postWebView)
                    }
                },
                onEditButtonPress = {
                    navController.currentBackStackEntry?.savedStateHandle?.set("post", post)
                    navController.navigate(NavHostRoute.postEdit)
                }
            )
        },
        bottomBar = {
